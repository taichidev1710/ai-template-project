...
## Trước khi viết code
1. Xác định feature.
   - Module CRUD **cơ bản** (list + form, không có hiển thị/hành vi biến đổi theo
     dữ liệu): **mở `src/features/users` và sao chép cấu trúc của nó**
     (`api/` → `hooks/` → `components/` → `pages/` → `index.ts`), hoặc chạy
     `npm run generate:feature <name>`. Bắt buộc theo quy ước Table ⇄ Grid + filter
     bar + View/Edit qua Modal — chi tiết ở `11-forms-tables.md`.
   - Module **đặc biệt** (hiển thị/hành động đổi theo tier, quyền, trạng thái...):
     xem `src/features/profile` làm ví dụ, được phép lệch khỏi chuẩn ở trên nhưng
     phải ghi rõ lý do.
