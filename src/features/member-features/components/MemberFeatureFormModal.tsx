...
import type { MemberFeatureInput, MemberFeatureItem } from '../types';

interface Props {
  open: boolean;
  initialValue?: MemberFeatureItem | null;
  confirmLoading?: boolean;
  onSubmit: (values: MemberFeatureInput) => void;
  onCancel: () => void;
}

const EMPTY: MemberFeatureInput = {
  key: '',
  name: '',
  description: '',
  icon: '',
  actions: [{ key: 'view', label: 'Xem' }],
  enabled: true,
  order: 0,
};

export function MemberFeatureFormModal({
  open,
  initialValue,
  confirmLoading,
  onSubmit,
  ...
  const [form] = Form.useForm<MemberFeatureInput>();
  ...
  return (
    <Modal
      open={open}
      title={isEdit ? t('memberFeature.editTitle') : t('memberFeature.createTitle')}
      ...
      destroyOnHidden
    >
      <Form<MemberFeatureInput> form={form} layout="vertical" onFinish={onSubmit} requiredMark={false}>
        <Form.Item
          name="key"
          label={t('feature.key')}
          rules={[{ required: true, pattern: /^[a-z][a-z0-9_]*$/ }]}
        >
          <Input disabled={isEdit} placeholder="e.g. booking" />
        </Form.Item>
